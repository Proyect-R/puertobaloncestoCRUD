...
class Model
{
    ...
    function modificarTodo($datos)
    {
        try {
            // Si viene el id de la tabla sigo adelante
            $campos = array_keys($datos);

            // Array para depurar los datos
            $setCampos = [];
            foreach ($campos as $campo) {
                // excluimos el id para la sentencia
                if ($campo !== 'idEntrenador') {
                    $setCampos[] = "`$campo` = :$campo";
                }
            }

            // Creamos la sentencia
            $sql = "UPDATE `puertobaloncesto`.`" . $this->table . "` SET " . implode(', ', $setCampos) . " WHERE idEntrenador = :id";
            ...
            // tratamos el id el ultimo
            $stmt->bindValue(':id', $datos['idEntrenador'], PDO::PARAM_INT);
